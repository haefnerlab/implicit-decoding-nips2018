...
params = BinarySparseCoding.ModelParams(G, N, pixel_noise_std, prior, pix);

offset = linspace(-2, 2, 5);
p = zeros(length(offset), T);
for i = 1:length(offset)
    fprintf('Running luminance level %d/%d\n', i, length(offset));
    p(i, :) = Decoding.OffsetNuisance(params, sig_eb, offset(i), -2, 2, T);
end
