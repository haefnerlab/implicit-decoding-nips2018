...
repeats = 50;
Templates = tools.StimGenerator(T,ang_val1,ang_val2*(params.n_neurons-1)/params.n_neurons);
fix_item = round(T/2); %The actual stimulus image
Im_selected = Templates(:,fix_item);
n_samples_set = 1;
figure();
thin_gap = 10;
burn_in=500;
angle_T = linspace(ang_val1,ang_val2*(T-1)/T,T);
angle_N = linspace(ang_val1,ang_val2*(N-1)/N,N);
...
run_ids = find(sum(ss{1}, 2) == 1);
cell_ids = arrayfun(@(i) find(ss{1}(i, :)), run_ids);
...
for j=1:3
    for i=1:length(n_samples_set)
        ...
        hold on
        plot(angle_T,ps{i}(ids(j),:),'-','linewidth',2,'Color',cols(j,:));
        set(gca, axformat{:})
        xlabel('Stim. Ori. (s)');
        ylim([0 0.08]);
        subplot(2,length(n_samples_set)+1,1+i+length(n_samples_set))
        hold on
        stem(angle_N,ss{i}(ids(j),:),'.','Color',cols(j,:),'linewidth',1);
        xlabel('Pref. Ori.');
        set(gca, axformat{:})
    end
end
for i=1:length(n_samples_set)
    subplot(2,length(n_samples_set)+1,i)
    hold on
    plot(angle_T,mean(ps{i}),'k-','linewidth',2);
    subplot(2,length(n_samples_set)+1,1+i+length(n_samples_set))
    hold on
    plot(angle_N,mean(ss{i}),'k-','linewidth',2);
    set(gca, axformat{:})
end
...
hold on
stem(angle_N,sum(x_samples,2),'.','Color',[.5 .5 .5],'linewidth',1);
